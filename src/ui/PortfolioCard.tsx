...
/**
 * A single-column list card for portfolio data.
...
export default function PortfolioCard({ data }: { data: PortfolioData }): React.JSX.Element {
  ...
  return (
    <Box flexDirection="column" paddingLeft={1} marginY={1}>
      <Box marginBottom={1}>
        <Text bold color="#00ffff">Portfolio Overview</Text>
      </Box>

      <Row label="$SIM Balance" value={formatNum(cash)} color="#00ffff" />
      <Row label="$NOVA Holdings" value={tokens > 0 ? formatNum(tokens) : "—"} color="#ff00ff" />
      <Row label="$NOVA Price" value={price > 0 ? formatPrice(price) : "—"} color="#ffff00" />
      ...
      {(earned > 0 || claimable > 0) && (
        <>
          <Row label="$ASTRA Earned" value={earned > 0 ? formatAstra(earned) : "—"} color="#ffff00" />
          <Row label="Claimable" value={claimable > 0 ? formatAstra(claimable) : "—"} color={claimable > 0 ? "#00ff00" : "gray"} />
        </>
      )}
    </Box>
  );
