  ...
  const elements = useMemo(() => {
    ...
    while (i < lines.length) {
      ...
      if (line.trimStart().startsWith(":::portfolio") || line.trimStart().startsWith(":::rewards")) {
        ...
        // If we hit end-of-input without closing :::, block is still streaming — hide it
        if (i >= lines.length) {
          break;
        }
        i++; // skip closing :::
        try {
          ...
          if (cardType === "portfolio") {
            ...
            result.push(<PortfolioCard key={result.length} data={data} />);
          } else {
            const data = JSON.parse(raw) as RewardsData;
            result.push(<RewardsCard key={result.length} data={data} />);
          }
        } catch {
          result.push(
            <Box key={result.length}>
      ...
      if (line.trimStart().startsWith("```")) {
        ...
        i++; // skip closing ```
        result.push(
          <Box key={result.length} marginLeft={2} marginY={0}>
      ...
      if (headerMatch) {
        result.push(
          <Box key={result.length} marginTop={result.length > 0 ? 1 : 0}>
      ...
      if (bulletMatch) {
        ...
        result.push(
          <Box key={result.length} marginLeft={indent * 2}>
            <Text>{"  ● "}</Text>
      ...
      // Empty line
      if (line.trim() === "") {
        result.push(<Box key={result.length}><Text> </Text></Box>);
        i++;
        continue;
      }
      ...
      // Regular paragraph
      result.push(
        <Box key={result.length} flexWrap="wrap">
          {renderInline(line)}
        </Box>,
      );
      i++;
